...
Наведений запит НЕ є покривний, бо:

- не виконується перша умова - друге поле фільтру `popularity` не є частиною індексу.
- проекція по замовченню включає всі поля та не виключає поле `_id`

Щоб зробити запрос покривним ми можемо: 

- створити індекс, що використовує всі поля фільтру 
- АБО модифікувати запит, щоб фільтр використовував тільки поля індексу
- в обох випадках проекція повинна включати тільки поля з індексу та виключати `_id`
